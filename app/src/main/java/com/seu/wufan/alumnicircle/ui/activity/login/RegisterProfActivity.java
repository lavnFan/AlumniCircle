...
public class RegisterProfActivity extends BaseActivity {
    ...
    @Bind(R.id.register_prof_et)
    EditText mrProfEt;

    @Override
    protected int getContentView() {
        return R.layout.activity_register_prof;
    }
    ...
    protected void initViews() {
        initToolBars();
        String prof = (getIntent().getExtras() == null) ? null : getIntent().getExtras().getString(EXTRA_Prof);
        TLog.i("prof:", prof);
        mrProfEt.setText(prof);
        mToolbarRightTv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Bundle bundle = new Bundle();
                bundle.putString(EXTRA_Prof, mrProfEt.getText().toString());
                Intent intent = new Intent();
                intent.putExtras(bundle);
                setResult(RegisterActivity.REQUESTCODE_Prof, intent);
                finish();
            }
        });
    }
